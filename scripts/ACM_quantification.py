...
def AllMatchedPoint(dirName):
	if not os.path.exists(dirName+'/s1.sam'):
	...
	with open(dirName+"/s3.txt","r") as obj:
		...
		for i in obj:
			listT = i.split()
			dictTemp[listT[0]] = listT[1:]
	with open(dirName+"/s1.sam",'r') as file_obj :
		r = file_obj.readline()
		Dict = {}
		while r:
			Dict.clear()
			...
			for i in length:
				if int(list1[3])<=int(i)<=(int(list1[3]) + 135):
			...
			while list1[2] == list2[2]:
				...
				for i in length:
					if int(list2[3])<=int(i)<=(int(list2[3]) + 135) :
						if  int(i) in Dict:
...
def ChangeBreakPoint(dirName):
	...
	with open(dirName+"/s4.txt","r") as obj:
		...
		for i in obj:
			...
			dictTemp[listT[0]] = listT[1]
	with open(dirName+"/s5.txt","r") as obj:
		r = obj.readline()
		while r:
			flag = 0
			dictW = {}
			...
			if flag == 1:
				#format is 'seqenceName {breakpoint : occurrence number}'
				with open(dirName+"/JunctionReadsCount.txt","a") as suobj:
					if dictW:
...
def Bowtie2ByFa(i,thread,rawData,dirName):
	subprocess.run("bowtie2 --local --no-unal --no-hd --quiet -a -p "+thread+" -x "+dirName+"/index"+str(i)+" -f -U "+rawData+" | grep --color=auto '125M' | grep --color=auto 'NM:i:0' > "+dirName+"/s"+str(i)+".sam ",shell=True)

#Call bowtie2 with the shortReads file in fastq format
def Bowtie2ByFq(i,thread,rawData,dirName):
	subprocess.run("bowtie2 --local --no-unal --no-hd --quiet -a -p "+thread+" -x "+dirName+"/index"+str(i)+" -U "+rawData+" | grep --color=auto '125M' | grep --color=auto 'NM:i:0' > "+dirName+"/s"+str(i)+".sam ",shell=True)
...
def Main():
	...
	with open('JunctionReadsCount.txt','r') as obj:
		for line in obj:
			if line:
				count=line.count(':')
				list=line.split()
