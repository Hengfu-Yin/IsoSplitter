...
def FindMatched(r,length,dirName):
	...
	if numberM >= 15 and int(listR[5][num+1:-1]) >= 15:	
		for i in length:
...
def Bowtie2ByFa(i,thread,rawData,dirName):
	subprocess.run("bowtie2 --local --no-unal --no-hd --quiet -a -p "+thread+" -x "+dirName+"/index"+str(i)+" -f -U "+rawData+" | grep --color=auto -Ew '[0-9]{2,3}M[0-9]{2,3}S' | grep --color=auto 'NM:i:0' > "+dirName+"/s"+str(i)+".sam ",shell=True)

#Call bowtie2 with the shortReads file in fastq format
def Bowtie2ByFq(i,thread,rawData,dirName):
	subprocess.run("bowtie2 --local --no-unal --no-hd --quiet -a -p "+thread+" -x "+dirName+"/index"+str(i)+" -U "+rawData+" | grep --color=auto -Ew '[0-9]{2,3}M[0-9]{2,3}S' | grep --color=auto 'NM:i:0' > "+dirName+"/s"+str(i)+".sam ",shell=True)
