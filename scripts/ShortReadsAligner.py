...
def CountSites(dirName):
	...
	with open(dirName+"/s1.sam","r") as obj:
		...
		while r:
			...
			while listR1[2] == listR2[2]:
				dictCount[listR1[2]]=dictCount[listR1[2]]+1
				FindMatched(r,length,dirName)
				r = obj.readline()
				listR2 = r.split()
				if not r:
	...
	for i in dictCount.keys():
		#s8.txt is total mapped reads to this transcript
		with open(dirName+"/s8.txt","a") as obj:
			obj.write(i+'\t'+str(dictCount[i])+'\n')
...
def ACM_count(dirName,longReadsFile):
	dictLength=dict()
	dictISR=dict()
	dictMapped=dict()
	with open(longReadsFile,'r') as obj:
		r=obj.readline()
		while not re.match('>', r):
			r = obj.readline()
			if not r:
	...
	with open(dirName+"/s7.txt",'r') as obj:
		for line in obj:
			if line:
				...
				dictISR[list[0]]=round(sum/count,4)
	with open(dirName+"/s8.txt",'r') as obj:
		for line in obj:
			if line:
				list=line.split()
				dictMapped[list[0]]=int(list[1])
	for i in dictISR.keys():
		if (i in dictLength.keys()) and (i in dictMapped.keys()):
			acm=dictISR[i]*round(dictMapped[i]/dictLength[i],4)
			with open(dirName+"/Average_counts_per.txt",'a') as sobj:
				sobj.write(i+'\t'+str(round(acm,4))+'\n')
...
def Main():
	...
	if not os.path.exists(dirName+'/ACMMapped.sam'):
		...
		ACM_count(dirName,args.longReadsFile)
